#include <stdio.h>
#define MAX_NUMBER_OF_ITEMS 10
#define MIN_NUMBER_OF_ITEMS 1

int main(void)
{
    const double MAX_INCOME = 400000.00;
    const double MIN_INCOME = 500.00;
    const double MIN_COST = 100.00;
    const int MIN_PRIORITY = 1;
    const int MAX_PRIORITY = 3;
    const int MIN_SELECTION = 0;
    const int MAX_SELECTION = 2;
    const char YES = 'y';
    const char NO = 'n';
    
    double income;
    int numOfItems;
    double total_cost = 0;
    
    int selection;
    int forecastYears;
    int forecastMonths;
    double monthsNeeded;
    
    int financingAvailable;
    int filterPriority;
    int foundItems;
    double filteredCost;
    
    int valid;
    int i;
    
    double cost[MAX_NUMBER_OF_ITEMS];
    int priority[MAX_NUMBER_OF_ITEMS];
    char finance[MAX_NUMBER_OF_ITEMS];
    
    printf("+--------------------------+\n");
    printf("+   Wish List Forecaster   |\n");
    printf("+--------------------------+\n\n");
    
    // input monthly income
    do {
        valid = 1;
        printf("Enter your monthly NET income: $");
        scanf("%lf", &income);
        if (income < MIN_INCOME) {
            valid = 0;
            printf("ERROR: You must have a consistent monthly income of at least $%.2lf\n\n", MIN_INCOME);
        }
        if (income > MAX_INCOME) {
            valid = 0;
            printf("ERROR: Liar! I'll believe you if you enter a value no more than $%.2lf\n\n", MAX_INCOME);
        }
    } while (valid == 0);
    
    printf("\n");
    
    
    // input number of wish list items
    do {
        valid = 1;
        printf("How many wish list items do you want to forecast?: ");
        scanf("%d", &numOfItems);
        if (numOfItems < MIN_NUMBER_OF_ITEMS || numOfItems > MAX_NUMBER_OF_ITEMS) {
            valid = 0;
            printf("ERROR: List is restricted to between %d and %d items.\n\n", MIN_NUMBER_OF_ITEMS, MAX_NUMBER_OF_ITEMS);
        }
    } while (valid == 0);
    
    
    // input details for each item
    for (i = 0; i < numOfItems; i++)
    {
        printf("\n");
        printf("Item-%d Details:\n", i + 1);
        
        // input item cost
        do {
            valid = 1;
            printf("   Item cost: $");
            scanf("%lf", &cost[i]);
            
            if (cost[i] < MIN_COST) {
                valid = 0;
                printf("      ERROR: Cost must be at least $%.2lf\n", MIN_COST);
            }
        } while (valid == 0);
        
        // input item priority
        do {
            valid = 1;
            printf("   How important is it to you? [1=must have, 2=important, 3=want]: ");
            scanf("%d", &priority[i]);
            
            if (priority[i] < MIN_PRIORITY || priority[i] > MAX_PRIORITY) {
                valid = 0;
                printf("      ERROR: Value must be between %d and %d\n", MIN_PRIORITY, MAX_PRIORITY);
            }
        } while (valid == 0);
        
        // input financing option
        do {
            valid = 1;
            printf("   Does this item have financing options? [y/n]: ");
            scanf(" %c", &finance[i]);
            
            if (finance[i] != YES && finance[i] != NO) {
                valid = 0;
                printf("      ERROR: Must be a lowercase \'y\' or \'n\'\n");
            }
        } while (valid == 0);
    }
    
    // calculate total cost and display item summary
    printf("\n");
    printf("Item Priority Financed        Cost\n");
    printf("---- -------- -------- -----------\n");
    
    for (i = 0; i < numOfItems; i++) {
        printf("%3d  %5d    %5c    %11.2lf\n", i + 1, priority[i], finance[i], cost[i]);
        total_cost = total_cost + cost[i];
    }
    printf("---- -------- -------- -----------\n");
    printf("                      $%11.2lf\n\n", total_cost);
    
    // wish list forecasting menu
    do {
        valid = 1;
        printf("How do you want to forecast your wish list?\n");
        printf(" 1. All items (no filter)\n");
        printf(" 2. By priority\n");
        printf(" 0. Quit/Exit\n");
        printf("Selection: ");
        scanf("%d", &selection);
        printf("\n");
        
        // invalid input
        if (selection < MIN_SELECTION || selection > MAX_SELECTION) {
            valid = 0;
            printf("ERROR: Invalid menu selection.\n\n");
        }
        
        // forecast for all items
        if (selection == 1) {
            monthsNeeded = total_cost / income + 0.5;
            forecastYears = (int)monthsNeeded / 12;
            forecastMonths = (int)(monthsNeeded + 0.5) % 12;
            
            printf("====================================================\n");
            printf("Filter:   All items\n");
            printf("Amount:   $%1.2lf\n", total_cost);
            printf("Forecast: %d years, %d months\n", forecastYears, forecastMonths);
            
            financingAvailable = 0;
            for (i = 0; i < numOfItems; i++) {
                if (finance[i] == YES) {
                    financingAvailable = 1;
                }
            }
            
            if (financingAvailable) {
                printf("NOTE: Financing options are available on some items.\n");
                printf("      You can likely reduce the estimated months.\n");
            }
            printf("====================================================\n\n");
        }
        
        // filter by priority and forecast
        if (selection == 2) {
            do {
                valid = 1;
                printf("What priority do you want to filter by? [%d-%d]: ", MIN_PRIORITY, MAX_PRIORITY);
                scanf("%d", &filterPriority);
                printf("\n");
                
                filteredCost = 0;
                foundItems = 0;
                for (i = 0; i < numOfItems; i++) {
                    if (priority[i] == filterPriority) {
                        filteredCost += cost[i];
                        foundItems++;
                    }
                }
                
                if (filterPriority < MIN_PRIORITY || filterPriority > MAX_PRIORITY) {
                    valid = 0;
                    printf("ERROR: Priority must be between %d and %d.\n\n", MIN_PRIORITY, MAX_PRIORITY);
                }
                
                else if (foundItems == 0) {
                    valid = 0;
                    printf("No items with priority %d were found.\n\n", filterPriority);
                }
            } while (!valid);
            
            if (foundItems > 0) {
                monthsNeeded = filteredCost / income + 0.5;
                forecastYears = (int)monthsNeeded / 12;
                forecastMonths = (int)(monthsNeeded + 0.5) % 12;
                
                printf("====================================================\n");
                printf("Filter:   by priority (%d)\n", filterPriority);
                printf("Amount:   $%1.2lf\n", filteredCost);
                printf("Forecast: %d years, %d months\n", forecastYears, forecastMonths);
                
                financingAvailable = 0;
                for (i = 0; i < numOfItems; i++) {
                    if (priority[i] == filterPriority && finance[i] == YES) {
                        financingAvailable = 1;
                    }
                }
                
                if (financingAvailable) {
                    printf("NOTE: Financing options are available on some items.\n");
                    printf("      You can likely reduce the estimated months.\n");
                }
                printf("====================================================\n\n");
            }
        }
    } while (!valid || selection != 0);
    
    printf("Best of luck in all your future endeavours!\n");
    
    return 0;
}
